...
function validateSQLite() {
  ...
  try {
    ...
    const practiceCount = db.prepare('SELECT COUNT(*) as count FROM best_practices').get() as { count: number };
    console.log(`✓ Best Practices: ${practiceCount.count}`);

    // Check metadata
    const metadata = db.prepare('SELECT * FROM export_metadata ORDER BY id DESC LIMIT 1').get() as any;
    if (metadata) {
    ...
    topLibs.forEach((lib: any) => console.log(`  - ${lib.name} (${lib.ecosystem})`));
    ...
    const highCount = db.prepare(
      "SELECT COUNT(*) as count FROM best_practices WHERE severity = 'high'"
    ).get() as { count: number };
    ...
    const reactRules = db.prepare(`
      ...
      WHERE l.name = 'react'
      LIMIT 10
    `).all() as { title: string; severity: string; version_range: string | null }[];

    // Filter by version using semver
    const version = '18.2.0';
    const matchingRules = reactRules.filter(rule => {
      if (!rule.version_range) return true;
      try {
